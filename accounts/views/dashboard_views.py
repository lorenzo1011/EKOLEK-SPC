...
def update_profile(request):
    """
    Update user's contact information (phone and email only).
    ...
    try:
        ...
        if Users.objects.filter(phone=phone).exclude(id=request.user.id).exists():
        ...
        masked_phone = f"{phone[:2]}{'*' * 7}{phone[-2:]}" if len(phone) >= 11 else phone
        
        logger.info(f"User {request.user.username} updated contact info - Phone: {phone}, Email: {email}")
